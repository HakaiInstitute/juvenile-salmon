...
ggsave(here::here("Time Series", "Time-series-manuscript","figs", "school_size_plot.png"), width = 7, height = 4)
```

```{r density histo}
mean_date <- so_catch_expanded %>% 
  group_by(region, year) %>% 
  summarise(mean = mean(yday, na.rm = TRUE)) %>% 
  spread(key = region, value = mean) %>% 
  mutate(diff = JS - DI)
  
avg_diff <- mean(mean_date$diff) 

mean_date <- mean_date %>%
  #select(-diff) %>% 
  gather(`DI`, `JS`, key = "region", value = "mean")

(histo <- so_catch_expanded %>% 
  ggplot(aes(yday, y = region, fill = region)) +
  geom_density_ridges(adjust = 4, na.rm = T, alpha = .8) +
  geom_vline(data=mean_date, aes(xintercept=mean, colour = region),
              size=.75) +
  facet_grid(year ~ .) +
...
ggsave(here("Time Series", "Time-series-manuscript", "figs", "density_histo.png"), width = 6, height = 4)
...
ggplot(data=prediction,aes(x = survey_date, y = y, group = region, shape = region))+
  ...
  geom_vline(data=mean_date, aes(xintercept=mean, linetype = region),
              size=.75) +
  facet_grid(year~.) +
...
ggsave(here("Time Series", "Time-series-manuscript", "figs", "log_cum_abund_curve.png"), width = 7, height = 4)
...
mean <- mean(skew_all)
se <- sd(skew_all)/ sqrt(3)

```

### Migration Abundance

To assess the abundance of each species I plot the average abundnace
