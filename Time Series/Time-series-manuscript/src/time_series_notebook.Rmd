...
cpue <-  select(survey_seines, survey_date, sampling_week, site_id, region, so_total, collection_protocol, survey_type) %>%
  ...
  filter(collection_protocol == "SEMSP", survey_type == "standard", site_id %in% c("D07", "D09", "D22", "D27", "D10", "D08", "D34",
                        "D20", "J03", "J02", "J09", "J11")) %>%
  mutate(year = year(survey_date)) %>%
  select(-site_id, -survey_date, - collection_protocol, -survey_type) %>% 
...
tidy_catch <- survey_seines %>%
  ...
  #TODO: Decide here if I want to drop_na() all seines that dont have complete enumeration, or if I want to filter seines to when there was > 0 sockeye. Currently I think it's better to drop_na() and so that species composition is calculated only from sets that had all species enumerated. The only drawback to this I can think of is sockeye proportions will be lower moving forward because we are going to conduct enumerations on seines that don't contain sockeye thereby  biasing future years low compared to 2015 and 2016
  drop_na() %>% 
  #filter(so_total > 0) %>% 
  mutate(year = year(survey_date)) %>% 
  # remove instances when there was not a complete enumeration of species in the seine
  drop_na() %>% 
...
parabola <-  select(survey_seines, survey_date, sampling_week, site_id, region, so_total) %>%
  ...
  drop_na() %>% 
  group_by(year, region, sampling_week) %>% 
  summarise(n = n(), so_total_mean = mean(so_total)) %>% 
  #remove instances when cpue was based on less than 3
  filter(n >= 3, so_total_mean >= 1)
  

ggplot(parabola, aes(x = sampling_week, y = log(so_total_mean), group = region, colour= region)) +
  geom_point() +
  stat_smooth(method = "smooth", formula = y ~ poly(x, 2), size = 1, se = F, n = 1) +
  facet_wrap(~year)
...
# Migration abundance is based on the catch per unit effort from seines that have complete enumeration of all species from in the seine because tidy_catch was created using drop_na() so that if a row had NA for any species totals then it was dropped. This relieves the concern that CPUE would be based on only the instances when sockeye were caught because in 2015 and to a lesser extent in 2016 we only enumerated the catch if sockeye were present. What we did was went back in to 2015 and 2016 data and looked at how many seines were conducted for a given survey. If there were multiple seines conducted, but no species composition recorded then we assumed that the catch of sockeye was zero for those sets because the protocol was to only retain and enumerate sets which had sockeye. We ccould not assume howver, that the catch of other species was zero because there may have been pink and chum which simply were not retained. In this way, sets which were not retained will have zeros recorded for sockeye totals permitting us to calculate CPUE of sockeye which includes every instance when the net was set. The abundance figures created below are based on the instances when sockeye were caught, or for seines where we recorded sockeye totals as 0. There is not perfect coherence then for the way we estimate abundance for species other than sockeye, which we already would expect given that we are targetting sockeye to set the net. The potential for systematic bias then comes from 2015 and 2016 when pink and chum abundance was only enumerated when they were caught with sockeye. Whereas in 2017 and planning for 2018, we will enumerate catches that did not contain sockeye. 
annual_cpue <- tidy_catch %>% 
 # filter(species == "SO" | n > 0) %>% 
