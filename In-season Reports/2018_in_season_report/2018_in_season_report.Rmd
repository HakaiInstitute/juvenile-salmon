...
di_2018_so_length_histo <- di_2018_so_length_histo %>% 
  mutate(higher_or_lower = if_else(fork_length < loess_annual, "lower", "higher"))

annual_di_length_histo %>% 
  mutate(yday = yday(survey_date)) %>% 
  filter(yday < 190, species == "SO") %>% 
  ggplot(aes(x = yday, y = fork_length)) +
  geom_point(data = di_2018_so_length_histo, aes(x = yday, y = fork_length, colour = higher_or_lower)) +
  geom_smooth(method = "loess", span = 1.)
```

```{r condition anomaly plot}
