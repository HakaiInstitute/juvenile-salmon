...
# Site success
ggplot(field_2018_sites_spp_summary, aes(x = survey_date, y = site_id, fill = n)) +
  geom_point(size = 3, shape = 21) +
  scale_fill_gradient(low = "white", high = "black") +
  facet_wrap( ~ species) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave(here("Internal Reports", "End of Field Season Reports", "2018", "Figures", "site_success.png"), height = 6, width = 6)

# Sockeye by zone
field_2018_zone_so_summary <- survey_seines_2018 %>% 
  select(survey_date, region, zone, so_taken) %>% 
  na.omit()

ggplot(field_2018_zone_so_summary, aes(x = survey_date, y = zone, colour = so_taken))+
  geom_point(size = 3) +
  scale_colour_gradient(high = "#132B43", low = "#56B1F7") +
  facet_wrap( ~ region, scales = "free_y") +
  scale_x_date(breaks = date_breaks("months"), labels = date_format("%b %d"), limits = as.Date(c("2018-05-01", "2018-08-22"))) +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
ggsave(here("Internal Reports", "End of Field Season Reports", "2018", "Figures", "so_zone_success.png"), height = 3, width = 6)
...
```{r YSI sampling study}

ysi <- gs_key("1yxot3J5Iltf66bB0KWgRxdSu58vpQvRk4cfm6p-TkVo", visibility = "private", lookup = F)

avg <- gs_read(ysi, ws="avg") %>% 
  filter(site_id == "D09_Ocgy_Near" | site_id == "D09_Ocgy" | site_id == "D09_Ocgy_Far")
avg$time.class = factor(avg$time.class, levels=c("Before","After"))
avg$site_id = factor(avg$site_id, levels=c("D09_Ocgy_Near","D09_Ocgy","D09_Ocgy_Far"))

# s_time <- ggplot(avg, aes(x=time.class, y=sal_avg)) +
#   geom_boxplot()
# s_time
# 
# t_time <- ggplot(avg, aes(x=time.class, y=temp_avg)) +
#   geom_boxplot()
# t_time

s_site <- ggplot(avg) +
  geom_boxplot(mapping = aes(x=site_id, y=sal_avg, color=time.class)) +
  scale_x_discrete(labels = c("Nearshore", "Standard", "Far Shore")) +
  labs(color="Sampling Period", x = "D09 Oceanography Site", y = "Average Salinity (ppt)")
s_site 
ggsave(here("Internal Reports", "End of Field Season Reports", "2018", "Figures", "YSI_salinity_D09_by_site.png"))

t_site <- ggplot(avg) +
  geom_boxplot(mapping = aes(x=site_id, y=temp_avg, color=time.class)) +
  scale_x_discrete(labels = c("Nearshore", "Standard", "Far Shore")) +
  labs(color="Sampling Period", x = "D09 Oceanography Site", y = "Average Temperature (°C)")
t_site 
ggsave(here("Internal Reports", "End of Field Season Reports", "2018", "Figures", "YSI_temp_D09_by_site.png"))
```