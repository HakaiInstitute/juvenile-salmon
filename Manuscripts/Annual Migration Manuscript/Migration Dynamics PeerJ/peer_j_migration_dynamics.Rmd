...
iqr_DI_allyears <- rbind(peak_dates, iqr_DI_tsa) #%>% 
  # mutate(q1 = format(q1 + as.Date("2018-01-01") -1, format = "%h %d")) %>% 
  # mutate(median = format(median + as.Date("2018-01-01") -1, format = "%h %d")) %>% 
  # mutate(q3 = format(q3 + as.Date("2018-01-01") -1, format = "%h %d"))

so_q1_DI_TSA <- as.numeric(iqr_DI_allyears %>% 
  filter(species == "SO" & year == "TSA") %>% 
  select(q1))
...
Migration timing in the Discovery Islands in 2018 did not differ from the time series average by more than a week for sockeye, pink, or chum. The peak migration date for sockeye in the Discovery Islands was on `r f(so_q2_DI_current)`, `r so_timing_diff` days `r so_later_or_earlier` than average of `r f(so_q2_DI_TSA)` (z-score = `r round(so_z_migration,2)`) (Figure \@ref(fig:mt)). The peak migration date for pink in the Discovery Islands was on `r f(pi_q2_DI_current)`, `r pi_timing_diff` days `r pi_later_or_earlier` than average of `r f(pi_q2_DI_TSA)` (z-score = `r round(pi_z_migration,2)`). The peak migration date for chum in the Discovery Islands was on `r f(cu_q2_DI_current)`, `r cu_timing_diff` days `r cu_later_or_earlier` than average of `r f(cu_q2_DI_TSA)` (z-score = `r round(cu_z_migration,2)`).
...
Table: (\#tab:mtdi) Migration timing statistics for the cumulative catch of sockeye, pink, and chum salmon in the Discovery Islands in 2018, compared to the time-series average.

  Species | Year      |            Q1|              Peak Date |              Q3
------- | --------- | ------------------:| ------------------:| ------------------:
Sockeye | Average   | `r f(so_q1_DI_TSA)`   | `r f(so_q2_DI_TSA)`   | `r f(so_q3_DI_TSA)`
&nbsp;  | `r current_year`  | `r f(so_q1_DI_current)` | `r f(so_q2_DI_current)`  | `r f(so_q3_DI_current)`
Pink    | Average   | `r f(pi_q1_DI_TSA)`   | `r f(pi_q2_DI_TSA)`   | `r f(pi_q3_DI_TSA)`
&nbsp;  | `r current_year`  | `r f(pi_q1_DI_current)`  | `r f(pi_q2_DI_current)`  | `r f(pi_q3_DI_current)`
Chum    | Average   | `r f(cu_q1_DI_TSA)`   | `r f(cu_q2_DI_TSA)`   | `r f(cu_q3_DI_TSA)`
&nbsp;  | `r current_year`  | `r f(cu_q1_DI_current)`  | `r f(cu_q2_DI_current)`  | `r f(cu_q3_DI_current)`
...
Pink salmon dominated the catch in the Discovery Islands and Johnstone Strait in 2018 making up `r pi_prop` percent of the catch while chum made up `r cu_prop` percent and sockeye `r so_prop` (Figure \@ref(fig:prop)).
