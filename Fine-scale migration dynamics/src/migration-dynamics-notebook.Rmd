...
hakai_so <- survey_seines_fish_stock_id %>% 
  filter(site_id == "D09") %>% 
  select(survey_date, site_id, species, fork_length, weight, stock_1, prob_1,
         ufn, semsp_id) %>% 
  mutate(org = "hakai")

same_date <- semi_join(hakai_so, dfo_data, by = "survey_date") %>% 
  filter(weight != "NA") %>% # removes fish that haven't been dissected
...
dfo_same_date <- semi_join(dfo_data, same_date, by = "survey_date") %>% 
  filter(prob_1 != "NA") %>% 
  group_by(survey_date) %>% 
  summarize(n = n())

hakai_same_date <- same_date %>% 
  group_by(survey_date) %>% 
  summarise(n_hakai = n())

dates_worth_comparing <- full_join(dfo_same_date, hakai_same_date, by = "survey_date") %>% 
  drop_na(n) %>% 
...
additional_hakai_fish <- same_date %>% 
  filter(survey_date %in% dates_worth_comparing)
```

Based on the seines that DFO has done stock IDs, there is opportunity to stock ID an additional `r nrow(additional_hakai_fish)` fish from the Hakai collection.
