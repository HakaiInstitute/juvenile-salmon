...
chilko_by_week <- chilko_and_other %>% 
  group_by(sampling_week, stock_group) %>% 
...
ggplot(chilko_by_week, aes(x = sampling_week, y = diff)) +
    geom_point() +
  ggtitle("Diff in lengths of Chilko vs other stocks")

chil <- as.numeric(chilko_by_week$Chilko)
oth <- as.numeric(chilko_by_week$all_others)

t.test(chil, oth, paired = FALSE)
```
...
Chilko %>% 
  ...
  drop_na() %>% 
  ggplot(aes(x = sampling_week, y = diff)) +
  geom_point() +
  geom_smooth() + 
  ggtitle("Diff in lengths of chilko between hakai and dfo")

#TODO: t-test to see if Chilko stocks from hakai are shorter than dfo
rm(stel)
rm(oth)
rm(Chilko)
rm(Chilko_by_week)
