...
I will try to do this will a chi square test.

To do that, I need to convert proportions to integers by multiplying them by 100.
```{r proportions test}
...
According to the chi square test for the entire seasons proportions, the two methods are not independent.
...
#### 1. Are the stellako fish caught by each organization different than each other?
...
```{r compare stellako from both orgs to all other stocks}
stellako <- okisollo %>% 
...
# Because the temporal sampling frequency is so different between stellako and other fish, I average lengths by week

stellako_by_week <- stellako %>% 
  group_by(year, stock_group, sampling_week) %>% 
  summarize(mean_fl = mean(fork_length, na.rm = T))

ggplot(stellako_by_week, aes(x = mean_fl, fill = stock_group)) +
  geom_density(alpha = 0.5) +
  ggtitle("Stellako sockeye length distribution from both organizations compared to all other stocks not including stellako")

#TODO: make sure that this t-test is taking into account sampling_week. I think I need to filter out events where there isn't a match.
t.test(mean_fl ~ stock_group, data = stellako_by_week)
...
season, we could be biased if the temporal sampling is not the same. __This needs
to be addressed.__ 
