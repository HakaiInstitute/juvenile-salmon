...
```{r 2016 annual gsi, fig.cap= "Genetic stock proportions of all stocks of sockeye captured in 2016 in Okisollo channel"}
...
shuswap_lengths_hakai_dfo_2016 <- Late_Shuswap_Portage %>% 
  ...
  mutate(diff = DFO - hakai) %>% 
  drop_na() %>% 
  mutate(numeric_sampling_week = c(1,2,3,4,5))
...
There is a significant trend in length difference of chum in 2015
...
all_CU_by_week_by_org_2016$numeric_sampling_week <- c(1,2,3,4,5,6)
# I noticed the difference in means of chilko lengths compared to all other
# lengths seemed to increase over the season so I plotted the difference over
# time and it is an intersting trend that I can't explain.
